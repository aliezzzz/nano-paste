...
const emit = defineEmits<{
  ...
  (e: "item-action", payload: { id: string; action: "copy" | "download" | "delete" | "favorite"; type: "text" | "file"; content?: string; fileId?: string; fileName?: string; isFavorite: boolean }): void;
}>();
...
function itemAction(payload: { id: string; action: "copy" | "download" | "delete" | "favorite"; type: "text" | "file"; content?: string; fileId?: string; fileName?: string; isFavorite: boolean }): void {
        ...
        <div class="host-header-actions">
          <div class="host-device-dropdown-wrap group">
            <div class="host-device-chip">
              <span id="connection-indicator" class="host-connection-dot status-dot" :class="connectionStatusMap[props.connectionStatus].color"></span>
              <span id="connection-status-text" class="host-connection-text">{{ connectionStatusMap[props.connectionStatus].text }}</span>
              <span class="host-device-count">
                <span>{{ props.activeDevices.length }}</span> 设备
                <svg class="host-device-count-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="host-device-dropdown">
              <div class="host-device-dropdown-card glass">
                <div class="host-device-dropdown-title">在线设备</div>
                <div class="host-device-list">
                  <template v-if="props.activeDevices.length > 0">
                    <div v-for="device in props.activeDevices" :key="device.deviceId" class="host-device-item">
                      <div>
                        <div class="host-device-item-name">{{ device.deviceName }}</div>
                        <div class="host-device-item-platform">{{ device.platform }}</div>
                      </div>
                      <span v-if="device.isCurrent" class="host-device-item-current">当前</span>
                    </div>
                  </template>
                  <div v-else class="host-device-item-empty">暂无在线设备</div>
                </div>
                <div class="host-device-dropdown-footer">
                  <button id="manage-devices-btn" type="button" class="host-device-manage-btn" @click="openDeviceManager">
                    管理设备
                  </button>
                </div>
              </div>
          ...
          <button id="open-config-btn" type="button" title="连接配置" aria-label="连接配置" class="host-icon-btn" @click="openConfig">
            <svg viewBox="0 0 1024 1024" class="host-icon-btn-icon" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          ...
          <div class="host-user-wrap group">
            <button type="button" title="用户菜单" aria-label="用户菜单" class="host-user-trigger">
              {{ userInitial }}
            </button>
                ...
                <div class="host-user-profile">
                  <div class="host-user-profile-row">
                    ...
                    <div class="host-user-name-wrap">
                      <div class="host-user-name">{{ props.username || "未命名用户" }}</div>
                    </div>
                  </div>
                </div>
                <button id="logout-btn" type="button" class="host-logout-btn" @click="logout">
                  <svg class="host-logout-btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="host-user-wrap group">
              ...
              <div class="host-user-dropdown">
                <div class="host-user-dropdown-card glass">
                  ...
                  <button id="logout-btn-mobile" type="button" class="host-logout-btn" @click="logout">
                    <svg class="host-logout-btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path>
                    </svg>
                    退出登录
