      ...
      <div id="items-list" class="h-full overflow-y-auto custom-scrollbar space-y-3">
        <div v-for="item in visibleItems" :key="item.id" class="bg-slate-900/85 border border-slate-700/50 rounded-lg p-4 hover:border-violet-500/30 transition-all group">
          <div class="flex gap-3">
            <!-- 左侧图标 -->
            <div class="w-10 h-10 rounded-lg bg-slate-800/70 flex items-center justify-center shrink-0 mt-0.5" v-html="item.iconSvg"></div>
            
            <!-- 右侧内容区 -->
            <div class="flex-1 min-w-0">
              <!-- 标题行：标题/文件名 + [类型] + [大小] + [★] -->
              <div class="flex items-center justify-between gap-3 mb-2">
                <div class="font-medium text-white truncate text-base flex-1">
                ...
                <div class="flex items-center gap-2 shrink-0">
                  ...
                  <!-- 文件大小（仅文件） -->
                  <span v-if="item.type === 'file' && item.fileSize" class="text-xs text-slate-400">
                    {{ formatBytes(item.fileSize) }}
                  </span>
                  <!-- 收藏按钮 -->
                  <button
                    class="w-7 h-7 rounded-lg transition-all flex items-center justify-center"
                    :class="item.isFavorite ? 'text-amber-300 bg-amber-500/15 hover:bg-amber-500/25' : 'text-slate-500 hover:text-amber-300 hover:bg-amber-500/10'"
              ...
              <div v-if="item.type === 'text'" class="mb-3">
                <p class="text-base text-slate-200 whitespace-pre-wrap line-clamp leading-relaxed">
                  {{ item.content || '无内容' }}
                </p>
              </div>
              ...
              <div class="flex items-center justify-between pt-2 border-t border-slate-800/50">
                ...
                <button 
                  ...
                  :class="item.type === 'text' ? 'text-violet-300 bg-violet-500/10 hover:bg-violet-500/20' : 'text-amber-300 bg-amber-500/10 hover:bg-amber-500/20'"
                  @click="emit('item-action', item.type === 'text' ? { id: item.id, action: 'copy', content: item.content ?? '' } : { id: item.id, action: 'download' })"
                >
                ...
                <div class="flex items-center gap-3">
                  <span class="text-xs text-slate-500">{{ new Date(item.createdAt).toLocaleString('zh-CN') }}</span>
                  <button 
                    v-if="!item.isFavorite"
                    class="flex items-center gap-1 px-2 py-1 rounded-md text-xs text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all"
                    title="删除"
                    @click="emit('item-action', { id: item.id, action: 'delete' })"
                  >
