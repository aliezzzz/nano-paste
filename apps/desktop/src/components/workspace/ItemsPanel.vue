  ...
  <div class="flex-1 flex flex-col min-h-0 p-4 sm:p-5">
    ...
    <div class="relative flex-1 min-h-0">
      <div id="items-list" class="h-full overflow-y-auto custom-scrollbar space-y-3">
        <div v-for="item in props.items" :key="item.id" class="bg-slate-900/85 border border-slate-700/50 rounded-lg p-4 hover:border-violet-500/30 transition-all group">
      ...
      <p id="items-empty" class="absolute inset-0 flex flex-col items-center justify-center text-slate-500" :class="props.loading || props.items.length > 0 ? 'hidden' : ''">
