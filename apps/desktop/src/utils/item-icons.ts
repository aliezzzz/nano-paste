...
export function getItemIconData(item: ItemDetail): ItemIconData {
  const svg =
    item.type === "text"
      ? (item.contentKind === "code" ? iconByKind.code : iconByKind.text)
      : iconByKind[
  ...
  return {
    ...
    cardBackground: primaryColor ? toAlphaColor(primaryColor, 0.02) : undefined,
  };
}
...
const iconByKind: Record<FileKind, string> = {
  ...
  code: '<svg class="w-7 h-7" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><rect x="8" y="10" width="48" height="44" rx="10" fill="#0891b2"/><path d="M20 24l-6 8 6 8" fill="none" stroke="#ecfeff" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><path d="M44 24l6 8-6 8" fill="none" stroke="#ecfeff" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><rect x="29" y="22" width="6" height="20" rx="3" fill="#67e8f9"/></svg>',
